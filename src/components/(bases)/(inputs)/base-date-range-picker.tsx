...
export function BaseDateRangePicker<T extends FieldValues>({
  ...
  return (
    <FormField
      ...
      name={fromFieldName}
      render={({ field }) => {
        const fromValue = field.value ? new Date(field.value) : undefined;
        const toValue = methods.watch(toFieldName)
          ? new Date(methods.watch(toFieldName))
          : undefined;
                  ...
                  >
                    ...
                    {dateRange?.from ? (
                      dateRange.to ? (
                        <>
                          {format(
                            new Date(dateRange.from.setUTCHours(3, 0, 0, 0)),
                            "dd 'de' MMMM 'de' yyyy",
                            {
                              locale: ptBR,
                            },
                          ...
                          -{" "}
                          {format(
                            new Date(dateRange.to.setUTCHours(3, 0, 0, 0)),
                            "dd 'de' MMMM 'de' yyyy",
                            {
                              locale: ptBR,
                            },
                    ...
                    <Calendar
                      ...
                      onSelect={(range) => {
                        if (range?.from) {
                          ...
                          field.onChange(format(fromDate, "yyyy-MM-dd"));
                        } else {
                          field.onChange("");
                        }
                        if (range?.to) {
                          ...
                          methods.setValue(
                            toFieldName,
                            format(toDate, "yyyy-MM-dd") as T[Path<T>],
                          );
                        } else {
                          methods.setValue(toFieldName, "" as T[Path<T>]);
                        }
                      }}
                      disabled={(date) => {
                        ...
                        if (isDateAvaliable && !isDateAvaliable(date))
                          return true;
                        return false;
                      }}
                      numberOfMonths={1}
